...
def sameclocktime_predicate(time_to_constant_dict, time_step, block_count, data_path, fold='0', setting='eval', ts=0):
	...
	for idx, time in enumerate(times_list):
		...
		for idx_2, time_2 in enumerate(times_list[idx+1:]):
			...
			if pd.to_datetime(time).hour == pd.to_datetime(time_2).hour:
				...
				k += 1
		k = 0
	write_lines_to_predicate_file(os.path.join(data_path, str(fold), setting, str(ts), "sameclocktime_obs.txt"), sameclocktime_predicate_lines)