...
readonly ADDITIONAL_PSL_OPTIONS='--postgres psl --int-ids'
readonly ADDITIONAL_LEARN_OPTIONS='-l org.linqs.psl.application.learning.weight.maxlikelihood.MaxPiecewisePseudoLikelihood -D votedperceptron.numsteps=100 -D votedperceptron.stepsize=1'
...
function runWeightLearning() {
   ...
   java -jar "${JAR_PATH}" --model "${BASE_NAME}.psl" --data "${BASE_NAME}-wl.data" ${ADDITIONAL_LEARN_OPTIONS} ${ADDITIONAL_PSL_OPTIONS} "$@"
   if [[ "$?" -ne 0 ]]; then
