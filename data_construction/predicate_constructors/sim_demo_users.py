...
def sim_demo_users_predicate(user_df, fold='0', phase='eval'):
    ...
    # take top 25 for each user to define pairwise blocks
    user_demo_similarity_block_frame = pd.DataFrame(index=user_df.index, columns=range(25))
    for u in user_df.index:
        user_demo_similarity_block_frame.loc[u, :] = user_similarity_df.loc[u].nlargest(25).index

    flattened_frame = user_demo_similarity_block_frame.values.flatten()
    user_index = np.array([[i] * 25 for i in user_demo_similarity_block_frame.index]).flatten()
